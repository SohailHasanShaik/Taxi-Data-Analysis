#What time of the day of the week the people tend to tip more
#This code will generate a CSV file for tip obtained in each hour for a week 
...
trip_count_schema = types.StructType([
    ...
    types.StructField('tips', types.FloatType())
    ...
    for company in companies:  
        ...
        for file, year, month  in available_files:              
            ...
            if os.path.exists(path): 
                ...
                data  = data.where(col('tips')>0).select('pickup_datetime','dropoff_datetime','tips')
                data = data.withColumn('weekday', fun.dayofweek(col('pickup_datetime')))\
                .withColumn('hour', fun.hour(col('pickup_datetime')))
                data = data.groupBy('weekday','hour').agg((avg(col('tips'))/4.3).alias('tips'))
                final_result = final_result.union(data)
                print("Wrote your file")
                
            else:
                print("file not found", file)
    final_result = final_result.groupBy('weekday','hour').agg(avg('tips').alias('tips'))

    output_path_tip_rush_hours = os.path.join(output_folder, 'tip_rush_hours')
    print("writng file to :", output_path_tip_rush_hours)
    final_result.repartition(1).write.option("header", "true").csv(output_path_tip_rush_hours, mode='overwrite')
